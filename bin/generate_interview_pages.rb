#!/usr/bin/env ruby
# DEPRECATED: This script is replaced by _plugins/generate_interviews.rb
...
interviews.each do |interview|
  ...
  File.open(path, 'w') do |f|
    ...
    f.puts '    {% if related_entry.links.size > 0 or conference_related.size > 0 %}'
    f.puts '      <section class="related-content">'
    f.puts '        <h2>Related Conference Videos</h2>'
    f.puts '        {% if related_entry.links.size > 0 %}'
    f.puts '          {% for link in related_entry.links %}'
    f.puts '            {% case link.kind %}'
    f.puts '            {% when "conference-bio" %}'
    f.puts '              <div class="video-meta">Interviewee Bio: <a href="{{ link.url }}">{{ link.label }}</a></div>'
    f.puts '            {% when "conference-presentation-page" %}'
    f.puts '              <div class="video-meta">Presentation Details: <a href="{{ link.url }}">{{ link.label }}</a></div>'
    f.puts '            {% when "conference-presentation-video" %}'
    f.puts '              <div class="video-meta">Presentation Video: <a href="{{ link.url }}">{{ link.label }}</a></div>'
    f.puts '            {% else %}'
    ...
    f.puts "    {% include schema-factory.html interview=interview asset=asset %}"
