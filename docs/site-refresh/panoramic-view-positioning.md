...
```mermaid
graph TD
    PROFILE[Professional Profile / Selected Resume] --> METHOD[Panoramic View]
    PROFILE --> PORTFOLIO[Portfolio]
    PROFILE --> HISTORY[Complete Career Timeline]
    PROFILE --> WRITING[Writing]
    PROFILE --> CONTACT[Contact]
    METHOD --> ENTERPRISE[Enterprise Archetype Case]
    METHOD --> TRANSITION[Founder-Transition Archetype Case]
    METHOD --> EXAMPLE[Neutral Worked Example]
    PORTFOLIO --> METHOD
    HISTORY --> PROFILE
    WRITING --> METHOD
    PROFILE --> HOME[Personal and Archive Hub]
    HOME --> ARCHIVE[Conversation Archive]
    ARCHIVE --> TIMELINE[Timeline]
    ARCHIVE --> SPEAKERS[Speakers]
