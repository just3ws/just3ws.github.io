...
2. **Strangler-Fig Decoupling & API Boundary Design**:
   At OneMain Financial, refactored high-volume financial customer acquisition flows spanning legacy database layers, multi-service Rails state machines, and third-party API supply chains: eliminating hidden lateral state mutations and eliminating cross-service blind spots.
...
| :--- | :--- | :--- | :--- |
| **Legacy Integration** | Plug modern SaaS into legacy EHRs without breaking live clinical operations. | 130+ clinical SaaS environments stabilized; strangler-fig API decoupling across financial monoliths. | 🟢 **Direct Match** |
...
1. **EMR-Bear 90-Day Risk Cartography & Clinical Governance (2026)**:
   - *Challenge*: Multi-tenant healthcare SaaS serving 130+ clinics faced operational risk during a founder exit.
   - *Action*: Reconstructed end-to-end dependency models (interaction surface to database locks) and instituted deployment release gates prior to HIPAA compliance review.
...
2. **OneMain Financial Acquisition Lane Realignment (2021–2026)**:
...
- **Outreach Angle**: Lead with common Groupon alumni context + direct evidence of plugging modern Rails interfaces into legacy 130+ clinic healthcare platforms without clinical downtime.
