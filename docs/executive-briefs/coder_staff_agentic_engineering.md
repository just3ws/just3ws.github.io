...
**Company:** Coder (Remote, US)  
**Core Domain:** Agent Harness, Tool Use Execution, Context Budget Management, Distributed Workflows  

---

## 🎯 Executive Summary & Mandate Alignment

Coder’s mandate: **evolving the agent harness, integrations, and long-running workflows that connect autonomous coding agents with real development environments**: aligns directly with my hands-on track record building production-grade Model Context Protocol (MCP) servers, bounded subagent architectures, and deterministic simulation engines.

I design systems where agent capabilities are **verifiable, context-budgeted, compliance-safe, and decoupled from model churn**.

---

## 🏗️ Direct Architectural Match & Evidence

### 1. Agent Harness, MCP Tooling & Execution Lifecycles
* **System Built (`Agent Tooling`):** Architected and operate three production MCP servers (`ctx-mcp`, `o2-mcp`, `llama-mcp`) exposing live system state, telemetry, and local inference as callable tools to agent runtimes.
...
## 💬 30-Second Interview Calibration
