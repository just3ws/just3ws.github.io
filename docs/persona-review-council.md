...
The full operating contract is maintained in `docs/agents/council-operating-model.md`, the cross-agent responsibility assignment matrix is defined in `docs/agents/agent-raci-matrix.md`, the SME working group delegation and multi-archive verification procedures are codified in `docs/sme-delegation-and-multi-archive-playbook.md`, and the adversarial audit findings and resolutions are documented in `docs/adversarial-content-audit-resolution.md`.
