...
```mermaid
flowchart LR
    A[Observe the wolf] --> B[Name the concrete location]
    B --> C[Ask for the huntsman]
    C --> D[Connect to the responsible owner]
    D --> E[Share evidence in that context]
    E --> F[Choose the next action]
    F --> G[Protect the village and learn]
    G --> A
```
