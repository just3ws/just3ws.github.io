...
```mermaid
flowchart TB
    R[The reference point] --> Ruby[`self`: the current object]
    R --> JS[`this`: the current execution context]
    R --> Unix[`$HOME`: the place to return to]
    R --> Towel[The towel: the thing that tells you where you are]
    Ruby --> H[Here]
    JS --> H
    Unix --> H
    Towel --> H
...
```mermaid
flowchart LR
    H[Here: current reference] --> M[Move toward there]
    M --> T[There: new experience]
    T --> I[Integrate what changed]
    I --> H2[Here: changed reference]
    H2 --> M
```
...
```mermaid
flowchart TD
    A[Basecamp: rest and orient] --> B[Choose the next bearing]
    B --> C[Move into the work]
    C --> D[Reach a temporary there]
    D --> E[Return with evidence]
    E --> F[Repair, integrate, and reorient]
    F --> A
```
